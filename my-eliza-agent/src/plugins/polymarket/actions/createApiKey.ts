  ...
  handler: async (
    ...
    try {
      ...
      try {
        ...
        // Try to derive existing API key first with timeout
        const deriveController = new AbortController();
        const deriveTimeout = setTimeout(() => deriveController.abort(), 30000); // 30 second timeout
        ...
        try {
          const deriveResponse = await fetch(`${clobApiUrl}/auth/derive-api-key`, {
            ...
            signal: deriveController.signal,
          });
        ...
        const createTimeout = setTimeout(() => createController.abort(), 30000); // 30 second timeout
